...
router.post('/anime', (req,res) => {
    //kondisi apabila body kosong
    if(!req.body.anime){
        res.status(400).send("DATA HARUS DIISI!!!")
        return;
    }

    const anime = {id: data.length + 1, nama: req.body.nama ,anime: req.body.anime};
    data.push(anime);
    res.send({
...
router.put('/anime/:id', (req,res) => {
    ...
    if(!req.body.anime && !req.body.nama){
        res.status(400).send("DATA HARUS DIISI!!!")
        return;
    };

    cariData.nama = req.body.nama;
